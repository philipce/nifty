...
public struct Matrix: CustomStringConvertible
{
	...
	/// Optional name of matrix for use in display
	...
	public init(size: Int, value: Double, name: String? = nil)
	{
		self.init(size: [size], value: value, name: name)						
	}
	...
	public init(size: Int, data: [Double], name: String? = nil)
	{
		self.init(size: [size], data: data, name: name)
	}
	...
	///		- name: optional name of matrix
	public init(data: [[Double]], name: String? = nil)
	{
		...
		precondition(size[0] * size[1] == numel, "Matrix dimensions must be consistent")
	...
	/// - Parameters:
	///		- copy: matrix to copy
	///		- rename: optionally specify new name, otherwise M.name is used
	public init(copy: Matrix, rename: String? = nil)
	{
		self.numel = copy.numel
		self.size = copy.size
		self.data = copy.data
		self.name = rename != nil ? rename : copy.name		
	}
	...
	public subscript(_ s: Int...) -> Double
	{
		get
		{
			...
			{	
				precondition(_isValidSubscript(s), "Invalid matrix subscripts: \(s)")
	...
	public var description: String
	{
		...
		var str = "\(self.size[0])-by-\(self.size[1]) matrix"
