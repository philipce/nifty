...
public func <= <T>(left: Matrix<T>, right: Matrix<T>) -> Matrix<Double> 
    where T: Comparable { return le(left, right) }
...
public func le<T>(_ A: Matrix<T>, _ B: Matrix<T>) -> Matrix<Double>
    where T: Comparable 
{
...
/// - Returns: matrix with ones where comparison is true and zeros elsewhere
public func le<T>(_ A: Matrix<T>, _ b: T) -> Matrix<Double>
    where T: Comparable 
{
    var m = [Double]()
    for i in 0..<A.count
...
///     - B: Matrix to compare against
/// - Returns: matrix with ones where comparison is true and zeros elsewhere
public func le<T>(_ a: T, _ B: Matrix<T>) -> Matrix<Double>
    where T: Comparable 
{
    var m = [Double]()
    for i in 0..<B.count
