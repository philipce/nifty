 ...
 *  log10_test.swift
 *
 *  This file tests the log10 function.
 *
...
class log10_test: XCTestCase 
{
    ...
    {        
        ...
        let v1_answer = Vector([0.0, 0.3010299, 0.477121])
        XCTAssert(isequal(log10(v1), v1_answer, within: 0.0001))
        
        // Test that log works for each element of the matrix
        let m1 = Matrix([[1.0, 2.0], [3.0, 4.0]])
        let m1_answer = Matrix([[0.0, 0.3010299], [0.477121, 0.6020599]])
        XCTAssert(isequal(log10(m1), m1_answer, within: 0.0001))
        
        // Test that log works for each element of the tensor
        let m = Matrix(2,3, [1.0, 2.0, 3.0, 1.0, 2.0, 3.0])
        let m_answer = Matrix(2,3, [0.0, 0.3010299, 0.477121, 0.0, 0.3010299, 0.477121])
        let tm = log10(Tensor(m))
